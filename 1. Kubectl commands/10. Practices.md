...
kubectl exec practice-pod -- cat /etc/os-release

# System uptime and load
kubectl exec practice-pod -- uptime

# Memory usage
kubectl exec practice-pod -- free -h

# Disk space usage
kubectl exec practice-pod -- df -h

# Current user and environment
kubectl exec practice-pod -- whoami
kubectl exec practice-pod -- env
...
#### Exercise 4: Deployment Rollback and Undo Practice
```bash
# 1. Create initial deployment
kubectl create deployment rollback-demo --image=nginx:1.20 --replicas=3

# 2. Check initial state and record it
kubectl get deployment rollback-demo
kubectl rollout history deployment/rollback-demo

# 3. Update deployment image (creates new revision)
kubectl set image deployment/rollback-demo nginx=nginx:1.21 --record

# 4. Check rollout status
kubectl rollout status deployment/rollback-demo

# 5. Scale deployment (creates another revision)
kubectl scale deployment rollback-demo --replicas=5
kubectl annotate deployment rollback-demo kubernetes.io/change-cause="Scaled to 5 replicas"

# 6. Check rollout history (should show 3 revisions)
kubectl rollout history deployment/rollback-demo

# 7. Get detailed info about specific revision
kubectl rollout history deployment/rollback-demo --revision=2

# 8. Current state check
kubectl get deployment rollback-demo
kubectl get pods -l app=rollback-demo

# 9. Undo to previous revision (rollback scaling)
kubectl rollout undo deployment/rollback-demo

# 10. Check after rollback
kubectl get deployment rollback-demo
kubectl rollout history deployment/rollback-demo

# 11. Rollback to specific revision (revision 1)
kubectl rollout undo deployment/rollback-demo --to-revision=1

# 12. Verify rollback to original state
kubectl get deployment rollback-demo
kubectl describe deployment rollback-demo | Select-String "Image:"

# 13. Check final history
kubectl rollout history deployment/rollback-demo
...
#### Exercise 5: Manual Scaling Rollback
```bash
# 1. Create deployment with known initial replica count
kubectl create deployment scale-demo --image=nginx:alpine --replicas=2
...
# 4. Manual rollback method 1: Scale back to original
kubectl scale deployment scale-demo --replicas=$INITIAL_REPLICAS
kubectl get deployment scale-demo

# 5. Scale to different value
kubectl scale deployment scale-demo --replicas=10

# 6. Manual rollback method 2: Use patch command
kubectl patch deployment scale-demo -p '{"spec":{"replicas":2}}'
kubectl get deployment scale-demo

# 7. Alternative: Use kubectl apply with YAML
...
kubectl get events --sort-by=.metadata.creationTimestamp | Select-String "failing-pod"
...
# 4. Look at events
kubectl get events --sort-by=.metadata.creationTimestamp | Select-String "resource-pod"
...
kubectl exec deployment/web-deployment -- env | Select-String "SERVER_NAME|CONFIG_"
...
for i in {1..12}; do
  ...
  kubectl exec file-config-pod -- findstr "server_name = updated-app" /etc/config/app.conf && break
  sleep 10
done
